...
$mutasi_id = isset($_POST['id']) ? intval($_POST['id']) : 0;
if ($mutasi_id <= 0) {
    die(json_encode(['success' => false, 'message' => 'ID Mutasi tidak valid.']));
}

// Ambil path file untuk dihapus
$sql_select = "SELECT path_lampiran FROM pegawai_mutasi WHERE id = ?";
$stmt_select = $conn->prepare($sql_select);
$stmt_select->bind_param("i", $mutasi_id);
$stmt_select->execute();
...
$sql_delete = "DELETE FROM pegawai_mutasi WHERE id = ?";
$stmt_delete = $conn->prepare($sql_delete);
$stmt_delete->bind_param("i", $mutasi_id);
