...
$sql_select = "SELECT path_lampiran FROM pegawai_surat_perintah WHERE id = ?";
$stmt_select = $conn->prepare($sql_select);
$stmt_select->bind_param("i", $surat_id);
$stmt_select->execute();
...
$sql_delete = "DELETE FROM pegawai_surat_perintah WHERE id = ?";
$stmt_delete = $conn->prepare($sql_delete);
$stmt_delete->bind_param("i", $surat_id);

if ($stmt_delete->execute()) {
    echo json_encode(['success' => true, 'message' => 'Surat perintah berhasil dihapus.']);
} else {
